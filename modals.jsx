...
/* ---------- Sacred Homas Modal (16 homas, single price) ---------- */
const SACRED_HOMAS = [
  'Rudra Homam',
  'Maha Mrutyinjaya Homam',
  'Kala Bhairava Homam',
...
const HOMA_PRICE = 15116;

function SacredHomasModal({ open, onClose }) {
  const [step, setStep] = useStateM(1);
  ...
  return (
    <Modal open={open} onClose={close} title={done ? 'Sankalpa Received' : 'Book Sacred Homa'} sub={done ? 'Confirmation' : `Step ${step} of 3`}>
      ...
      {!done && step === 1 && (
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: '1px solid var(--line)', background: 'rgba(255,122,46,0.05)', padding: '18px 24px', marginBottom: 26 }}>
            <span style={{ fontFamily: 'var(--f-display)', fontSize: 10, letterSpacing: '0.28em', textTransform: 'uppercase', color: 'var(--ivory-faint)' }}>All Sacred Homas</span>
            <span style={{ fontFamily: 'var(--f-display)', fontSize: 24, color: 'var(--gold)' }}>₹{HOMA_PRICE.toLocaleString('en-IN')} each</span>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, maxHeight: 420, overflowY: 'auto' }}>
            {SACRED_HOMAS.map(h => (
              <button key={h} onClick={() => setHoma(h)} style={{
                padding: '18px 14px', textAlign: 'left', minHeight: 70,
                background: homa === h ? 'rgba(255,122,46,0.08)' : 'transparent',
                border: `1px solid ${homa === h ? 'var(--gold)' : 'var(--line-soft)'}`,
                color: 'var(--ivory)', cursor: 'pointer', fontFamily: 'inherit',
                display: 'flex', alignItems: 'center',
              }}>
                <span style={{ fontFamily: 'var(--f-display)', fontSize: 12, letterSpacing: '0.03em', lineHeight: 1.4 }}>{h}</span>
              </button>
            ))}
          </div>
          <div style={{ marginTop: 24, display: 'flex', justifyContent: 'flex-end' }}>
            <button className="btn solid" disabled={!homa} onClick={() => homa && setStep(2)} style={{ opacity: homa ? 1 : 0.45 }}>
              Continue<span className="arrow"></span>
            </button>
          </div>
        </div>
      ...
      {!done && step === 3 && (
        <div>
          <div style={{ background: 'rgba(255,122,46,0.05)', border: '1px solid var(--line)', padding: 32, marginBottom: 28 }}>
            <div style={{ fontFamily: 'var(--f-display)', fontSize: 10, letterSpacing: '0.3em', color: 'var(--gold)', textTransform: 'uppercase', marginBottom: 16 }}>Your Sankalpa</div>
            <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: 14, fontSize: 15 }}>
              <div style={{ color: 'var(--ivory-faint)' }}>Homa</div><div>{homa}</div>
              <div style={{ color: 'var(--ivory-faint)' }}>Devotee</div><div>{form.name} {form.gotra && <span style={{ color: 'var(--gold)' }}>· {form.gotra} gotra</span>}</div>
              <div style={{ color: 'var(--ivory-faint)' }}>Nakshetra</div><div>{form.nakshetra}</div>
              <div style={{ color: 'var(--ivory-faint)' }}>Date</div><div>{form.date}</div>
              <div style={{ color: 'var(--ivory-faint)' }}>WhatsApp</div><div>{form.whatsapp}</div>
              <div style={{ color: 'var(--ivory-faint)' }}>Amount</div><div style={{ color: 'var(--gold)', fontFamily: 'var(--f-display)' }}>₹{HOMA_PRICE.toLocaleString('en-IN')}</div>
            </div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <button className="btn ghost" onClick={() => setStep(2)}>← Back</button>
            <button className="btn solid" onClick={() => setDone(true)}>
              Pay ₹{HOMA_PRICE.toLocaleString('en-IN')} via Razorpay
              <span className="arrow"></span>
            </button>
          </div>
          <div style={{ marginTop: 16, fontFamily: 'var(--f-mono)', fontSize: 10, letterSpacing: '0.2em', color: 'var(--ivory-faint)', textAlign: 'center' }}>
      ...
      {done && (
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          ...
          <p style={{ color: 'var(--ivory-dim)', maxWidth: 460, margin: '0 auto', lineHeight: 1.6 }}>
            The priests will perform <span style={{ color: 'var(--gold)' }}>{homa}</span> in your name.
            A confirmation receipt has been sent to your email and WhatsApp.
          </p>
          <div style={{ marginTop: 36 }}>
            <button className="btn" onClick={close}>Close</button>
