        ...
        <div className="grid-auto-4" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 18 }}>
          {POOJA_TIMES.map((p, i) => {
            ...
            <div key={i} className={`card pooja-card reveal delay-${i % 4}${isShivaratri ? ' maha-shivaratri-card' : ''}`} style={{ padding: '40px 24px', textAlign: 'center', flex: '1 1 calc(25% - 18px)', minWidth: 220, display: 'flex', flexDirection: 'column', position: 'relative' }}>
              ...
              <p style={{ fontSize: 14, color: isShivaratri ? 'var(--ivory-dim)' : 'var(--ivory-faint)', fontStyle: 'italic', lineHeight: 1.5, flex: 1, position: 'relative' }}>{p.desc === 'Prasadam offering (Daily 1kg)' ? <span style={{ fontSize: 16 }}>Prasadam offering<br/>(Daily <b>1</b>kg)</span> : p.name === 'Visesha|Pujas' ? <><span>{p.desc}</span><br/><span style={{ color: 'var(--gold)', fontStyle: 'normal' }}>1. Nandishwara Puja — ₹1,111</span></> : p.desc}</p>
...
const FESTIVALS = [
  ...
  {
    image: 'assets/Shiva_Parvati.jpeg',
...
const PAST_FESTIVALS = [
  ...
  },
  {
    ...
    desc: 'Completing 25 Years Silver Jubilee of service to the temple with Nakshatra Sukta Sri Rudra Parayanam Puja - 2x',
